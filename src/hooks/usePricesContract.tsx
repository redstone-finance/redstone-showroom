...
export const usePricesContract = (
  ...
  connector: ContractConnector<PricesContractAdapter>,
  ...
  const getPrices = async (fromContract: boolean) => {
    await performContractAction(async (adapter: PricesContractAdapter) => {
  ...
  const performContractAction = async (
    callback: (adapter: PricesContractAdapter) => void
  ) => {
    if (network) {
      try {
        startMockLoader();
